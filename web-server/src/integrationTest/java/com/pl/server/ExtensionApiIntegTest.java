 ...
 * @since 01/12/14.
 */
public class ExtensionApiIntegTest extends BaseServerIntegTest {
    ...
    public void testIsAlreadySaved() throws Exception {
        PagedResponse<ArticleResult> response = extensionApi.isAlreadySaved(GetByUrlRequest.of("url2"));
        verifyResponse(0, response);

        response = extensionApi.isAlreadySaved(GetByUrlRequest.of("http://stackoverflow.com/a/21023161"));
        List<ArticleResult> results = verifyResponse(1, response);
        assertThat(results.get(0).getItem().getTitle(), equalTo("Copying local files with curl"));

    }

    protected List<ArticleResult> verifyResponse(int size, PagedResponse<ArticleResult> response) {
